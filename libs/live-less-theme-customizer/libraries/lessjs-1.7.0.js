...
function loadStyles(modifyVars) {
    ...
    for (var i = 0; i < styles.length; i++) {
        ...
        if (style.type.match(typePattern)) {
            ...
            $(document).trigger('loadStyles.lessjs');
        }
    }
}
...
function loadStyleSheet(sheet, callback, reload, remaining, modifyVars) {
    ...
    loadFile(sheet.href, null, function(e, data, path, newFileInfo, webInfo) {
        ...
        if (data) {
          if (jQuery('body').data('less-postfix-localstorage-name')) {
            try {
              var additional = localStorage.getItem(jQuery('body').data('less-postfix-localstorage-name'));
              if (additional) {
              data += additional;
              }
            }
...
less.modifyVars = function(record) {
    ...
    $(document).trigger('modifyVars.lessjs');
};
